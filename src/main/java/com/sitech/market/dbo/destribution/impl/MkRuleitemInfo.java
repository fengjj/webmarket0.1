package com.sitech.market.dbo.destribution.impl;

import java.util.List;
import java.util.Map;

import com.sitech.market.busiobject.basicclass.dbclass.DBMysqlClass;
import com.sitech.market.dbo.destribution.inter.IMkRuleitemInfo;

public class MkRuleitemInfo extends DBMysqlClass implements IMkRuleitemInfo {
	
	/**
	 * Create on 2015-5-11 
	 * @author: yugl
	 * @Title: selectMkRuleitemInfo 
	 * @Description: 查询规则明细
	 * @param map
	 * @return 
	 * @version 1.0 
	 * update_data:       update_author:       update_note:
	 */
	@Override
	public List selectMkRuleitemInfo(Map map){
		return baseDao.queryForList("MkRuleitemInfo.selectMkRuleitemInfo", map);
	}
	
	/**
	 * Create on 2015-5-11 
	 * @author: nidw
	 * @Title: selectMkRuleitemInfo 
	 * @Description: 查询规则明细
	 * @param map
	 * @return 
	 * @version 1.0 
	 * update_data:       update_author:       update_note:
	 */
	@Override
	public List selectMkRuleitemInfoForRuleType(Map map){
		return baseDao.queryForList("MkRuleitemInfo.selectMkRuleitemInfoForRuleType", map);
	}
	
	/**
	 * Create on 2015-6-19 
	 * @author: yugl
	 * @Title: selectMkRuleitemInfoById 
	 * @Description: 根据主键查询
	 * @param itemId
	 * @return 
	 * @version 1.0 
	 * update_data:       update_author:       update_note:
	 */
	@Override
	public Map selectMkRuleitemInfoById(String itemId){
		return (Map)baseDao.queryForObject("MkRuleitemInfo.selectMkRuleitemInfoById", itemId);
	}
	
	/**
	 * Create on 2015-5-22 
	 * @author: lixd_bj
	 * @Title: selectRuleItemContentInfoBySerialId 
	 * @Description: 根据规则序列号查询奖项等明细信息
	 * @param map
	 * @return 
	 * @version 1.0 
	 * update_data:       update_author:       update_note:
	 */
	@Override
	public List selectRuleItemContentInfoBySerialId(Map map){
		return baseDao.queryForList("MkRuleitemInfo.selectRuleItemContentInfoBySerialId", map);
	}
	
	/**
	 * Create on 2015-6-16 
	 * @author: yugl
	 * @Title: insertMkRuleitemInfo 
	 * @Description: 新增规则明细
	 * @param map 
	 * @version 1.0 
	 * update_data:       update_author:       update_note:
	 */
	@Override
	public void insertMkRuleitemInfo(Map map) {
		baseDao.insert("MkRuleitemInfo.insertMkRuleitemInfo",map);
	}
	
	/**
	 * Create on 2015-6-16 
	 * @author: yugl
	 * @Title: updateMkRuleitemInfo 
	 * @Description: 修改规则明细
	 * @param map 
	 * @version 1.0 
	 * update_data:       update_author:       update_note:
	 */
	@Override
	public void updateMkRuleitemInfo(Map map) {
		baseDao.update("MkRuleitemInfo.updateMkRuleitemInfo",map);
	}
	
	/**
	 * Create on 2015-6-17 
	 * @author: yugl
	 * @Title: selectMkRuleitemInfoForCodeName 
	 * @Description: 关联codename表查询活动规则明细
	 * @param map
	 * @return 
	 * @version 1.0 
	 * update_data:       update_author:       update_note:
	 */
	@Override
	public List selectMkRuleitemInfoForCodeName(Map map){
		return baseDao.queryForList("MkRuleitemInfo.selectMkRuleitemInfoForCodeName", map);
	}
	
	/**
	 * Create on 2015-6-17 
	 * @author: yugl
	 * @Title: deleteMkRuleitemInfoById 
	 * @Description: 根据主键删除
	 * @param itemId 
	 * @version 1.0 
	 * update_data:       update_author:       update_note:
	 */
	@Override
	public void deleteMkRuleitemInfoById(String itemId) {
		baseDao.delete("MkRuleitemInfo.deleteMkRuleitemInfoById",itemId);
	}
	
	/**
	 * Create on 2015-6-19 
	 * @author: lixd_bj
	 * @Title: deleteMkRuleitemInfoByRuleSerialId 
	 * @Description: 根据ruleSerialIdList删除规则明细
	 * @param itemId 
	 * @version 1.0 
	 * update_data:       update_author:       update_note:
	 */
	@Override
	public void deleteMkRuleitemInfoByRuleSerialId(Map map) {
		baseDao.delete("MkRuleitemInfo.deleteMkRuleitemInfoByRuleSerialId",map);
	}
	/**
	 * Create on 2015-8-13
	 * @author: zhangchen
	 * @Title: selectRuleItemInfoBySerialId 
	 * @Description: 根据规则序列号查询奖项
	 * @param map
	 * @return 
	 * @version 1.0 
	 * update_data:       update_author:       update_note:
	 */
	@Override
	public List selectRuleItemInfoBySerialId(Map map){
		return baseDao.queryForList("MkRuleitemInfo.selectRuleItemInfoBySerialId", map);
	}

	/**
	 * Create on 2015-8-11 
	 * @author: wangpei
	 * @Title: copyMkRuleitemInfoByItemIdMap 
	 * @Description: 根据itemId复制规则明细
	 * @param itemId 
	 * @version 1.0 
	 * update_data:       update_author:       update_note:
	 */
	public void copyMkRuleitemInfoByItemId(Map map){
		baseDao.insert("MkRuleitemInfo.copyMkRuleitemInfoByItemId",map);
	};
}
